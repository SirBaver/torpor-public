...
### L134 -- Un runner qui supprime son repertoire RocksDB puis sort via process::exit sans avoir joint ses run_loops cree une course glibc-dependante : PASS sur l hote, SIGABRT en container (2026-06-11)

**Contexte :** audit pre-publication, build a froid du QUICKSTART dans un container vierge (rust:latest, glibc plus recente que l hote). `run-all.sh` : S12 `fail` au round 2, `pass` au round 3 -- flaky inter-environnements. Sur l hote, s12-runner sortait 0 mais imprimait `pthread lock: Invalid argument` sur stderr (signature `PthreadCall` de RocksDB port_posix -> abort()).

**Observation :** s12_runner faisait `remove_dir_all(db_root)` alors que les 6 run_loops (tasks tokio spawnees par `Scheduler::register`) detenaient encore leurs `Arc<ContentStore>`/`Arc<CausalLog>`, puis `std::process::exit` depuis `#[tokio::main]` -- aucun shutdown du runtime. Les threads background C++ de RocksDB (flush/compaction, relances par les erreurs sur fichiers supprimes) coursent alors les destructeurs atexit ; l issue de la course depend de la glibc -- d ou un flaky invisible sur la machine de dev. Trois faits structurels : (1) le `Scheduler` ne joint ni ne droppe rien tout seul -- ses JoinHandle sont prives, la seule API publique qui joint une run_loop est `evict_agent` ; (2) `drop(senders)` cote runner ne suffit PAS, le Scheduler garde ses propres clones de senders ; (3) un sleep apres drop (pattern p10_s3, 300 ms) signale la fin sans la garantir. Meme pattern dangereux trouve dans s11, p10_s3, p10_s5 ; s10 joignait deja ses tasks mais supprimait la DB encore ouverte. sef1_runner portait deja la sequence canonique (drop tx -> join -> drop store/log -> exit) -- c est le modele a copier.

**Regle generale :** dans un runner Tokio+RocksDB, la sequence de fin est : joindre les run_loops (via `evict_agent`, ou en conservant les JoinHandle et en les await-ant apres fermeture des channels) -> dropper TOUS les Arcs store/log -> seulement ensuite `remove_dir_all` -> `process::exit`. « Exit 0 + verdict pass » ne valide pas le shutdown : la course peut etre perdue ailleurs (glibc, charge) ; un message `pthread lock` sur stderr est un abort RocksDB latent, pas du bruit. Un build a froid en environnement vierge est precisement le detecteur de cette classe de bugs.

**Code adapte :** use case -- poc/runtime/src/bin/{s12_runner,s11_runner,p10_s3_runner,p10_s5_runner,s10_runner}.rs (shutdown ordonne). Aucun fichier du coeur runtime modifie.

**Reference :** poc/runtime/src/bin/s12_runner.rs (bloc shutdown), poc/runtime/src/bin/sef1_runner.rs (sequence canonique preexistante), poc/runtime/src/scheduler.rs (register : handles prives ; evict_agent : seule jointure publique). Voisins : L120 (un vert sans oracle execute ne prouve rien), L122 (valider le rendu reel).

### L135 -- Le QUICKSTART se valide depuis un CLONE de HEAD en environnement vierge, pas depuis l arbre de travail : `git clone` d un depot local prend HEAD, et c est la seule maniere de detecter les chiffres de validation figes et les copies de code qui ont derive (2026-06-11)

**Contexte :** meme audit pre-publication. Le QUICKSTART promettait « 53 tests verts » (chiffre fige au PoC E2E du 2026-05-16, devenu tres inferieur a la realite), `cargo build --workspace` depuis la racine (pas de Cargo.toml a la racine), aucun prerequis libclang, et aucune procedure pour la branche seL4 (l image Docker `rust-root-task-demo` se construit depuis un clone upstream gitignore -- URL et rev n etaient documentes nulle part dans le depot public). En outre `poc/benchmarks/src/bin/redb_p3a.rs` (copie workspace de poc/redb-p3a/src/main.rs, hors workspace) avait derive : imports de traits redb 4.x manquants -> le workspace ne compilait PLUS a HEAD, invisible depuis l arbre de travail ou des builds incrementaux passaient.

**Observation :** trois mecanismes de derive independants convergent vers le meme angle mort : (1) un chiffre de validation code en dur dans la doc ne suit pas le code ; (2) un fichier duplique (copie workspace vs crate standalone) derive silencieusement quand un seul des deux est mis a jour ; (3) une dependance d infrastructure gitignoree (clone upstream, image Docker locale) est invisible dans le depot publie. Aucun des trois n est detectable par les tests locaux. Piege supplementaire du harnais : `git clone /chemin/local` clone HEAD -- les correctifs non commites de l arbre de travail ne sont PAS dans le clone ; valider des correctifs au QUICKSTART exige de les committer d abord (ou de monter l arbre, ce qui n est plus un build a froid).

**Regle generale :** avant publication, derouler le QUICKSTART depuis un clone frais de HEAD dans un container vierge, sans intervention manuelle, et traiter chaque ecart comme un correctif de doc ou de code -- jamais comme une particularite d environnement. Les chiffres attendus (« N tests verts ») doivent etre regeneres a cette occasion. Toute copie de code intentionnelle (workspace vs standalone) et toute dependance gitignoree (toolchain, image) doivent etre soit documentees avec URL+rev epingles, soit eliminees.

**Code adapte :** use case -- QUICKSTART.md (cd poc, libclang, section seL4 avec rev upstream epingle, compte de tests), poc/benchmarks/src/bin/redb_p3a.rs (imports traits redb 4.x realignes sur poc/redb-p3a). Aucun fichier du coeur runtime modifie.

**Reference :** QUICKSTART.md, poc/benchmarks/src/bin/redb_p3a.rs vs poc/redb-p3a/src/main.rs, poc/sel4-hello (Makefiles -> image `rust-root-task-demo`, upstream github.com/seL4/rust-root-task-demo rev 7dcc192b). Voisins : L121 (triage CVE verrouille par tests fail-closed), L134 (bug revele par le meme build a froid).
