...
- [x] **Agents de rôle `andragogue` + `marketing`** *(2026-06-06)* : deux agents de référence dans `agents/`, câblés sur les garde-fous épistémiques (régimes R1/R2, substrat vs applicative, frontière LLM, F1/L68 — jamais survendre ni simplifier au prix de la fausseté). Enregistrés dans `CLAUDE.md`. Livrables produits : `docs/demo/demo-tui-script.md` (script + checklist anti-survente, marketing) et `docs/onboarding-parcours.md` (parcours 6 étapes, andragogue).
...
- [x] **wiki/ — Bundle exportable (Chantier 4)** *(2026-06-02)* : 7 fichiers dans `wiki/`. 00 (tout public), 01 (technique), 02 (use-cases régime-taggés), 03 (red team — stub structuré), 04 (direction seL4), 05 (glossaire), 06 (références). Chantiers 1 et 2 du briefing déjà couverts (catalogue régime-taggé depuis `a092354`, S15 depuis `89dc011`).
