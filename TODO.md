...
- [x] **Sauvegarde hors-machine** *(2026-06-10)* : les 3 dépôts poussés sur remotes GitHub **privés** (clé SSH de compte). Préalable réglé : blob 22 GB (benchmark P3a, absent de HEAD) purgé de l'historique source (`git filter-repo`, pack 372 MiB → 6,2 MiB, 3 SHA cités mis à jour). **Publication différée** *(décision utilisateur 2026-06-10 : « pas prêt à publier en vrai »)* — les dépôts restent privés ; triggers en aval (position seL4, article) dormants, conforme à leur design.
- [x] **Audit pré-publication (6 points)** *(2026-06-11)* : (1) historique git scanné — gitleaks propre sur les 2 dépôts publics, chemin absolu + note éditoriale corrigés à HEAD, `reference_responses.jsonl` conservé (bénin, requis par ADR-0021) ; (2) build à froid container — QUICKSTART corrigé (cd poc, libclang, section seL4 avec upstream épinglé, 164 tests) + workspace réparé (redb_p3a imports) + course shutdown RocksDB corrigée dans 5 runners (L134, L135) ; (3) licences redb-fork restaurées (MIT/Apache upstream + README provenance + exception README) ; (4) triage CVE verrouillé — 3 tests fail-closed ajoutés (winch/component model/WASI), `cargo audit` re-exécuté : 15 advisories, ensemble identique au triage B-1, addendum RUSTSEC-2026-0021 tracé ; (5) note hardware-scope §3.1 : déjà corrigée (commit 1919752) ; (6) posture de maintenance ajoutée aux README publics. **À refaire le jour de la publication : `cargo audit`.**
...
- [x] **Bundle exportable (Chantier 4)** *(2026-06-02)* : bundle de 7 fichiers — 00 (tout public), 01 (technique), 02 (use-cases régime-taggés), 03 (red team — stub structuré), 04 (direction seL4), 05 (glossaire), 06 (références). Chantiers 1 et 2 du briefing déjà couverts (catalogue régime-taggé depuis `8e3eeb0`, S15 depuis `f7dc893`).
