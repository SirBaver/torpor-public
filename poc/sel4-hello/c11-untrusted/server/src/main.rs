//! Jalon C.11 — Server : Phase A (commit loop, K=2) ou Phase B (verify K=2)
//!
//! K=2 : le module OOB commit 1 fois, le module loop commit 1 fois = 2 commits au total.
//!
//! Phase A :
//!   Reçoit commits des deux runtimes via ring SPSC (séquentiels), stocke dans redb/virtio-blk.
//!   Badge AGENT_A_ID=1 → commit ; ORACLE_BADGE=0xC11FE → query seq.
//!
//! Phase B :
//!   Après init, attend VERIFY_BADGE=0xC11FF du superviseur.
//!   Vérifie que TABLE_JOURNAL_A a K=2 entrées (clés 0 et 1).
...
const ORACLE_BADGE: u64 = 0xC11FE;
const VERIFY_BADGE: u64 = 0xC11FF;
const INIT_BADGE: u64 = 0xC11000;
...
fn commit_to_redb(db: &Database, ring_va: *mut RingBuffer) -> u64 {
    ...
    if data_len == 0 || data_len > 4092 {
        sel4::debug_println!("[C11] server: payload ring invalide (data_len={}), commit ignoré", data_len);
        return 0;
    }
...
fn oracle_query(db: &Database) -> u64 {
    ...
    // La table peut ne pas exister si aucun commit n'a encore eu lieu.
...
// Vérifie que K=2 entrées sont présentes dans TABLE_JOURNAL_A (clés 0 et 1).
...
pub fn main() -> ! {
    sel4::debug_println!("[C11] server: démarrage");

    let ep = sel4::cap::Endpoint::from_bits(EP_SLOT);

    // Init IPC : dma_paddr + phase
    let (_msg, badge) = ep.recv(());
    assert_eq!(badge, INIT_BADGE, "[C11] server: badge init inattendu");
    let (dma_paddr, phase) = sel4::with_ipc_buffer(|buf| {
        (buf.msg_regs()[0] as usize, buf.msg_regs()[1])
    });
    sel4::debug_println!("[C11] server: dma_paddr=0x{:08x} phase={}", dma_paddr, phase);

    HalImpl::init(DMA_SIZE, DMA_VA_BASE, dma_paddr);
    let blk_va = find_virtio_blk().expect("[C11] server: aucun virtio-blk");
    sel4::debug_println!("[C11] server: virtio-blk à VA=0x{:08x}", blk_va);
    ...
    sel4::debug_println!("[C11] server: virtio-blk {} secteurs", blk.capacity());

    let storage = if phase == 1 { BlockStorage::new_reopen(blk) } else { BlockStorage::new(blk) };
    let db = Database::builder()
        .set_cache_size(1024 * 1024)
        .create_with_backend(storage)
        .expect("[C11] server: Database::create_with_backend echoue");
    sel4::debug_println!("[C11] server: redb ouvert (cache 1MB, phase={})", phase);
    ...
    if phase == 1 {
        // Phase B : attendre VERIFY_BADGE, vérifier K=2 commits, suspendre
        loop {
            let (_msg_info, badge) = ep.recv(());
            if badge == VERIFY_BADGE {
                let (verified, seq_a) = verify_k_commits(&db);
                sel4::debug_println!(
                    "[C11] server Phase B: verified={} seq_a={} K={}",
                    verified, seq_a, K
                );
                sel4::with_ipc_buffer_mut(|buf| {
    ...
    sel4::debug_println!("[C11] server Phase A: ring_va=0x{:x}", ring_va as usize);
    loop {
        let (_msg_info, badge) = ep.recv(());
        if badge == ORACLE_BADGE {
            let seq_a = oracle_query(&db);
            sel4::debug_println!("[C11] server: oracle → seq_a={}", seq_a);
            sel4::with_ipc_buffer_mut(|buf| {
        ...
        if badge == AGENT_A_ID {
            ...
            sel4::debug_println!("[C11] server: commit → seq={}", seq);
            sel4::with_ipc_buffer_mut(|buf| {
                buf.msg_regs_mut()[0] = seq;
                sel4::reply(buf, sel4::MessageInfo::new(0, 0, 0, 1));
            });
        } else {
            sel4::debug_println!("[C11] server: badge inconnu {}", badge);
            sel4::with_ipc_buffer_mut(|buf| {
                sel4::reply(buf, sel4::MessageInfo::new(0, 0, 0, 0));
            });
        }
