#!/usr/bin/env python3
"""Test runner C.6-crash — un kill_point à la fois.
...
IMAGE = os.environ.get("IMAGE", "/poc/sel4-hello/c6-crash/build/image-kp1.elf")
...
print(f"[C6-crash] KP={KP} IMAGE={IMAGE}")
print(f"[C6-crash] Attente de {EXPECTED} (timeout {TIMEOUT}s)...")
sys.stdout.flush()
...
if idx == 0:
    ...
    print(f"\n[C6-crash] FAIL: QEMU terminé sans {EXPECTED}", file=sys.stderr)
    sys.exit(1)
else:
    print(f"\n[C6-crash] FAIL: timeout {TIMEOUT}s sans {EXPECTED}", file=sys.stderr)
    sys.exit(1)
