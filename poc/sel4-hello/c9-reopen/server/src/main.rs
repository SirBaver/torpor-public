//! Jalon C.9 — Server : Phase A (commit loop) ou Phase B (verify)
//!
...
const ORACLE_BADGE: u64 = 0xC9FE;
const VERIFY_BADGE: u64 = 0xC9FF;
const INIT_BADGE: u64 = 0xC9_0000;
...
fn commit_to_redb(db: &Database, ring_va: *mut RingBuffer) -> u64 {
    ...
    if data_len == 0 || data_len > 4092 {
        ...
        sel4::debug_println!("[C9] server: payload ring invalide (data_len={}), commit ignoré", data_len);
        return 0;
    }
...
pub fn main() -> ! {
    sel4::debug_println!("[C9] server: démarrage");

    let ep = sel4::cap::Endpoint::from_bits(EP_SLOT);

    // Init IPC : dma_paddr + phase
    let (_msg, badge) = ep.recv(());
    assert_eq!(badge, INIT_BADGE, "[C9] server: badge init inattendu");
    let (dma_paddr, phase) = sel4::with_ipc_buffer(|buf| {
        (buf.msg_regs()[0] as usize, buf.msg_regs()[1])
    });
    sel4::debug_println!("[C9] server: dma_paddr=0x{:08x} phase={}", dma_paddr, phase);

    HalImpl::init(DMA_SIZE, DMA_VA_BASE, dma_paddr);
    let blk_va = find_virtio_blk().expect("[C9] server: aucun virtio-blk");
    sel4::debug_println!("[C9] server: virtio-blk à VA=0x{:08x}", blk_va);
    ...
    sel4::debug_println!("[C9] server: virtio-blk {} secteurs", blk.capacity());
    ...
    let db = Database::builder()
        ...
        .expect("[C9] server: Database::create_with_backend echoue");
    sel4::debug_println!("[C9] server: redb ouvert (cache 1MB, phase={})", phase);
    ...
    if phase == 1 {
        ...
        loop {
            ...
            if badge == VERIFY_BADGE {
                ...
                sel4::debug_println!(
                    "[C9] server Phase B: verified={} seq_a={} K={}",
                    verified, seq_a, K
                );
                sel4::with_ipc_buffer_mut(|buf| {
    ...
    sel4::debug_println!("[C9] server Phase A: ring_va=0x{:x}", ring_va as usize);
    loop {
        let (_msg_info, badge) = ep.recv(());
        if badge == ORACLE_BADGE {
            let seq_a = oracle_query(&db);
            sel4::debug_println!("[C9] server: oracle → seq_a={}", seq_a);
            sel4::with_ipc_buffer_mut(|buf| {
        ...
        if badge == AGENT_A_ID {
            ...
            sel4::debug_println!("[C9] server: commit → seq={}", seq);
            sel4::with_ipc_buffer_mut(|buf| {
                buf.msg_regs_mut()[0] = seq;
                sel4::reply(buf, sel4::MessageInfo::new(0, 0, 0, 1));
            });
        } else {
            sel4::debug_println!("[C9] server: badge inconnu {}", badge);
            sel4::with_ipc_buffer_mut(|buf| {
                sel4::reply(buf, sel4::MessageInfo::new(0, 0, 0, 0));
            });
        }
