// Runtime C.9 — heap minimal (pas d'alloc dans le runtime Rust pur)
...
// 64 KB : suffisant pour sel4_panicking (catch_unwind) sans alloc réel
...
fn entrypoint() -> ! {
    ...
    match catch_unwind(main) {
        Ok(never) => never,
        Err(_) => abort!("runtime main() panicked"),
    }
}
