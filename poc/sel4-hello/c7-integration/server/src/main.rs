...
pub fn main() -> ! {
    sel4::debug_println!("[C7] server: démarrage");

    let ep = sel4::cap::Endpoint::from_bits(EP_SLOT);
    let mut store = Q3cStore::new();

    // Annoncer les VAs des rings (pour débogage)
    sel4::debug_println!(
        "[C7] server: ring_a VA={:#x}, ring_b VA={:#x}",
        get_ring_va_for_agent(0) as usize,
        get_ring_va_for_agent(1) as usize,
    );

    // Boucle serveur : recv(badge) → dispatch par agent_id → process ring → reply
    loop {
        let (_msg_info, badge) = ep.recv(());

        // Dispatch : badge = agent_id, ring_index = agent_id - 1
        let (agent_id, ring_va) = if badge == AGENT_A_ID {
            ...
            sel4::debug_println!("[C7] server: badge inconnu {}, ignoré", badge);
        ...
        sel4::debug_println!(
            "[C7] server: agent={} commit seq {} → {}  (blobs={}, headers={})",
            agent_id,
            seq_before,
            seq_after,
            store.blobs.len(),
