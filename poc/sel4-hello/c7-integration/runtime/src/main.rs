...
//!   4. Appelle ep.call() (commit-cap badgée agent_id) → serveur indexe par (agent_id, k)
//! Après le run WASM : signal done_nfn, suspend.
//!
...
//! Le badge sur la commit-cap (slot 1) IS l'agent_id — le runtime n'a pas besoin de le
//! connaître explicitement (ADR-0044 §D2).
...
pub fn main() -> ! {
    sel4::debug_println!("[C7] runtime: démarrage");
    ...
    linker
        .func_wrap("env", "emit", |caller: wasmtime::Caller<'_, EmitCtx>| -> i32 {
            ...
            if !ok1 || !ok2 || !ok3 {
                sel4::debug_println!("[C7] emit: ring buffer trop petit");
                return -1;
            }
    ...
    sel4::debug_println!("[C7] runtime: module WASM instancié");
    ...
    sel4::debug_println!("[C7] runtime: WASM run() terminé");
