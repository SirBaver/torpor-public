//! Jalon C.10-crash — Root task superviseur (atomicité crash fenêtre W→X)
//!
//! PHASE=0 (Phase A — crash dans fenêtre de remap) :
//!   Spawn server(phase=0) + runtime_crash.
//!   Runtime fait K=1 commit via emit(), puis crash dans fenêtre de remap W→X :
//!     - unmap une page JIT (frame_unmap)
//!     - signal suspend_nfn + tcb_suspend
//!   Superviseur attend suspend_nfn → runtime "crashé".
//!   Oracle query (ORACLE_BADGE) → server répond avec seq_a.
//!   Vérifie seq_a == K :
//!     PASS → imprime KP_WX_PASS puis C10_CRASH_PASS
//!     FAIL → imprime C10_CRASH_FAIL: seq_a=X expected K=Y
//!
//! PHASE=1 (Phase B — D-reopen) :
//!   Spawn server(phase=1) sur le même disk.img.
//!   Envoie VERIFY_BADGE → server vérifie K=1 entrées.
//!   Imprime C10_CRASH_REOPEN_PASS ou C10_CRASH_REOPEN_FAIL.
//!
...
const ORACLE_BADGE: sel4::Badge = 0xC10FE;
...
const RUNTIME_SLOT_NFN:    u64 = 2; // suspend_nfn (write_only)
...
/// Spawne le runtime crash (identique à spawn_runtime_wx de c10-wx).
///
/// CNode runtime size_bits=8 :
///   Slot 1 : EP commit (badge=AGENT_A_ID)
///   Slot 2 : suspend_nfn (write_only) — signal avant crash simulé
///   Slot 3 : TCB
///   Slot 4 : VSpace (pour wasmtime_mprotect)
///   Slots 5..132 : 128 caps frames JIT (état initial RW+EXECUTE_NEVER)
fn spawn_runtime_crash(
    allocator: &mut ObjectAllocator,
    ...
    sel4::debug_println!(
        "[C10-crash] {} frames JIT pré-mappées RW+XN à VA 0x{:08x}",
    ...
    // Slot 2 : suspend_nfn (write_only)
    runtime_cnode
        .absolute_cptr_from_bits_with_depth(RUNTIME_SLOT_NFN, CHILD_CNODE_SIZE_BITS_RUNTIME)
        .mint(&cnode.absolute_cptr(suspend_nfn), sel4::CapRights::write_only(), 0)
        .unwrap();

    // 5. Configurer le TCB (fault_ep = NULL : pas de test négatif ici)
    let tcb = allocator.allocate_fixed_sized::<sel4::cap_type::Tcb>();
    tcb.tcb_configure(
        sel4::init_thread::slot::NULL.cptr(),
        runtime_cnode,
...
fn main(bootinfo: &sel4::BootInfoPtr) -> sel4::Result<Never> {
    sel4::debug_println!("=== OS-pour-IA : C.10-crash superviseur (PHASE={}) ===", PHASE);
    ...
    sel4::debug_println!("[C10-crash] DMA: {} pages à paddr=0x{:08x}", DMA_PAGES, dma_paddr);

    let mmio_frames =
        allocator.allocate_device_frames(bootinfo, VIRTIO_MMIO_BASE_PHYS, VIRTIO_MMIO_PAGES);
    sel4::debug_println!("[C10-crash] MMIO: {} pages à 0x{:08x}", VIRTIO_MMIO_PAGES, VIRTIO_MMIO_BASE_PHYS);
    ...
    // Oracle cap (badge=ORACLE_BADGE) : superviseur → serveur pour query seq_a
    ...
    if PHASE == 0 {
        // ── Phase A : K=1 commit + crash dans fenêtre de remap ────────────────
        ...
        // suspend_nfn : le runtime signale ici avant de se suspendre (crash simulé)
        ...
        sel4::debug_println!("[C10-crash] Phase A: server spawné");
        ...
        sel4::debug_println!("[C10-crash] Phase A: server init OK");

        let runtime_image = File::parse(RUNTIME_ELF_CONTENTS).unwrap();
        let _runtime_tcb = spawn_runtime_crash(
            &mut allocator,
            &runtime_image,
            ring_a_orig,
            endpoint,
        ...
        sel4::debug_println!("[C10-crash] Phase A: runtime spawné (K={})", K);

        // ── Attendre suspend_nfn : runtime a crashé dans la fenêtre de remap ──
        suspend_nfn.wait();
        sel4::debug_println!("[C10-crash] suspend_nfn reçu : runtime crashé dans fenêtre de remap W→X");

        // ── Oracle query : vérifier que seq_a == K (store intact) ─────────────
        let _oracle_reply = oracle_ep.call(sel4::MessageInfo::new(0, 0, 0, 0));
        let seq_a = sel4::with_ipc_buffer(|buf| buf.msg_regs()[0]);

        sel4::debug_println!(
            "[C10-crash] Phase A: oracle → seq_a={} (attendu K={})",
            seq_a, K
        );

        if seq_a == K {
            // Store intact malgré le crash dans la fenêtre de remap
            sel4::debug_println!("KP_WX_PASS");
            sel4::debug_println!("C10_CRASH_PASS");
        } else {
            sel4::debug_println!(
                "C10_CRASH_FAIL: seq_a={} expected K={}",
                seq_a, K
            );
        }
        ...
        sel4::debug_println!("[C10-crash] Phase B: server spawné");
        ...
        let _init_reply = init_ep.call(sel4::MessageInfo::new(0, 0, 0, 2));
        sel4::debug_println!("[C10-crash] Phase B: server init OK (redb rouvert sur disk.img)");
        ...
        sel4::debug_println!(
            "[C10-crash] Phase B: verified={} seq_a={} K={}",
            verified, seq_a, K
        );

        if verified == K && seq_a == K {
            sel4::debug_println!("C10_CRASH_REOPEN_PASS");
        } else {
            sel4::debug_println!(
                "C10_CRASH_REOPEN_FAIL: verified={} seq_a={} expected K={}",
                verified, seq_a, K
            );
        }
    }
