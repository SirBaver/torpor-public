// Wasmtime custom platform W^X pour seL4 AArch64 — C.11
//
// Identique à c10-wx/runtime/src/platform.rs
...
pub unsafe extern "C" fn wasmtime_mmap_new(
    ...
    unsafe {
        ...
        if end > JIT_POOL_PAGES * PAGE_SIZE {
            sel4::debug_println!(
                "[C11] wasmtime_mmap_new: pool épuisé (aligned={} + size={} > {})",
    ...
    unsafe {
        ...
        for i in 0..n_pages {
            ...
            if want_exec {
                ...
                let _ = want_write; // satisfaire le compilateur
                frame_cap.frame_map(
                    vspace,
                    page_va,
...
pub unsafe extern "C" fn wasmtime_longjmp(_jmp_buf: *const u8) -> ! {
    panic!("wasmtime trap (longjmp) dans le runtime C.11");
}
