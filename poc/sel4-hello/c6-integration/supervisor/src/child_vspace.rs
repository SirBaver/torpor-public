...
//   - Le ring buffer (page après l'IPC buffer) — partagé entre server et runtime
//
// Retourne : (vspace, ipc_buffer_addr, ipc_buffer_cap, ring_va)
...
pub(crate) fn create_child_vspace<'a>(
...
) -> (sel4::cap::VSpace, usize, sel4::cap::Granule, usize) {
    ...
    // Footprint étendu : image + IPC buffer + ring buffer
    let extended_footprint =
        image_footprint.start..(image_footprint.end + 2 * GRANULE_SIZE);
    ...
    // Ring buffer — juste après l'IPC buffer
    let ring_va = image_footprint.end + GRANULE_SIZE;
    ring_frame
        .frame_map(
            child_vspace,
            ring_va,
    ...
    (child_vspace, ipc_buffer_addr, ipc_buffer_cap, ring_va)
}
