// Utilitaires mémoire pour C.4 :
//   - map_ram_pages               : N frames RAM contigus → paddr retournée
//   - map_device_pages_contiguous : N frames device (MMIO) contigus à partir de target_paddr
//
// Pattern : rust-sel4/examples/root-task/serial-device + platform.rs C.3

use sel4::{CapRights, ObjectBlueprint, ObjectBlueprintArch, VmAttributes, init_thread};

pub const PAGE_SIZE: usize = 4096;

/// Alloue N SmallPages RAM depuis le plus petit Untyped non-device ≥ N×4KB.
/// Les mappe à [va_base .. va_base + N×4KB). PTs créés à la volée.
/// Retourne paddr du premier frame (frames physiquement contigus).
...
/// Cherche l'Untyped device couvrant [target_paddr .. target_paddr + N×4KB),
/// trimme son watermark jusqu'à target_paddr si nécessaire, retypes N SmallPages,
/// et les mappe à [va_base .. va_base + N×4KB). PTs créés à la volée depuis
/// le plus grand Untyped non-device.
pub fn map_device_pages_contiguous(
    ...
    // Avancer le watermark de l'Untyped jusqu'à target_paddr si nécessaire.
    // On consomme 2 slots temporaires (réutilisés à chaque itération).
    if ut_paddr != target_paddr {
        trim_untyped(ut, ut_paddr, target_paddr, next_slot);
    }

    // Retype N pages depuis le watermark actuel (= target_paddr)
    let frames_base = *next_slot;
    ut.untyped_retype(&page_bp, &cnode_abs, frames_base, n_pages)
        .expect("retype SmallPages device échoué");
    *next_slot += n_pages;

    // PT provider : plus grand Untyped non-device
...
// Avance le watermark d'un Untyped de ut_paddr jusqu'à target_paddr
// en retypant des Untypeds "déchets" de taille décroissante.
// Utilise 2 slots temporaires réutilisés à chaque itération (O(1) slots nets).
fn trim_untyped(ut: sel4::cap::Untyped, ut_paddr: usize, target_paddr: usize, next_slot: &mut usize) {
    let cnode = init_thread::slot::CNODE.cap();
    let cnode_abs = cnode.absolute_cptr_for_self();
    ...
    *next_slot += 2;
    ...
    while cur_paddr != target_paddr {
        ...
        ut.untyped_retype(&ObjectBlueprint::Untyped { size_bits }, &cnode_abs, slot_b, 1)
            .unwrap();
        // 1er tour : slot_a est vide, delete retourne une erreur ignorée
        let _ = rel_a.delete();
