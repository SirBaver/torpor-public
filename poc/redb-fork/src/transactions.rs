...
struct TableNamespace<'db> {
    open_tables: HashMap<String, &'static core::panic::Location<'static>>,
...
impl TableNamespace<'_> {
    ...
    fn inner_open<K: Key + 'static, V: Value + 'static>(
        ...
        self.open_tables
            .insert(name.to_string(), core::panic::Location::caller());
...
impl SavepointTransactionState {
    ...
    fn apply_on_abort(&mut self, tracker: &TransactionTracker) {
        ...
        for (savepoint, transaction) in core::mem::take(&mut self.created_persistent) {
            tracker.deallocate_savepoint(savepoint, transaction);
        }
...
impl WriteTransaction {
    ...
    pub fn restore_savepoint(&mut self, savepoint: &Savepoint) -> Result<(), SavepointError> {
        ...
        {
            let tables = self.tables.lock();
            for page in tables.allocated_pages.lock().reset() {
...
impl Drop for WriteTransaction {
    fn drop(&mut self) {
        if !self.completed && !false && !self.mem.storage_failure() {
